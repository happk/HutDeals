...
- **項分類（新架構，取代舊「組分類」）**：分類改以「項」為單位（項＝前端一個選項組），值為 大/小/個人/特殊比薩、比薩、義大利麵/飯、副食、飲料。
  - 飲料／副食／義大利麵飯 ← **看該項每個候選品名**（單一詞彙表 `script/lib/categories.py`）；大／小／個人／特殊比薩 ← **讀該項的官方組標題**（候選品名不含尺寸）。
  - **不再用官網組型 `main`/`second` 決定分類**（官網不嚴謹：標「請選擇1份飲料」的組內混有雞軟骨/薯金幣；標「請選擇1份副食」的組內有整組可樂）。加購（add）項不參與分類。
  - 混類項同時掛多類（如 `["副食","飲料"]`），兩種篩選都找得到；前端組標題顯示聯集「副食/飲料」。
